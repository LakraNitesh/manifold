...
(function () {
    ...
    <main class="wrap bench" id="bench">
      ...
      <aside class="side" id="side">
        <div class="quest-head">
          <span class="label" style="color:var(--accent)">Your quest — in order</span>
  ...
  // Engine enforces order. Page calls q.done(id) when a 'do' step's condition is met.
  // q.onPredict(id, chosenIndex) fires for predictions; q.onAll() when everything done.
  MF.questCtl = function (container, steps, hooks = {}) {
    ...
    function render() {
      steps.forEach((s, i) => {
        const el = els[i];
        el.className = 'qstep ' + (state[i] === 'done' ? 'done' : (i === cur ? 'now' : 'future'));
        el.querySelector('.qmark').textContent = state[i] === 'done' ? '✓' : (i + 1);
        const opts = el.querySelector('.qopts');
        if (opts) opts.style.display = (i === cur && state[i] !== 'done') ? 'flex' : 'none';
      });
      const doneN = state.filter(s => s === 'done').length;
      countEl.textContent = `${doneN}/${steps.length} complete`;
    }

    steps.forEach((s, i) => {
      const el = document.createElement('div');
      el.className = 'qstep future';
      el.innerHTML = `<div class="qtop"><span class="qmark">${i + 1}</span><span class="qtitle"></span></div>
        <p class="qdesc"></p>`;
    ...
    const api = {
      ...
      isDone: (id) => state[steps.findIndex(s => s.id === id)] === 'done',
        ...
        state[i] = 'done';
        els[i].classList.add('justdone');
        cur++;
        render();
        if (!silent) MF.toast('STEP ' + (i + 1) + ' — DONE', steps[i].title);
        if (cur < steps.length) {
          setTimeout(() => MF.toast('NEXT UP', steps[cur].title, 2200), silent ? 300 : 1400);
        } else {
          hooks.onAll && hooks.onAll();
        }
